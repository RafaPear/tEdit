...
object Color {
    ...
    fun String.stylize(color: List<ColorCode>, reset: Boolean = true): String =
        if (color.isEmpty()) this
        else if (reset) RESET_ALL.code + make(color) + this + RESET_ALL.code
        else make(color) + this

    fun Char.stylize(color: List<ColorCode>, reset: Boolean = true): String =
        toString().stylize(color, reset)
